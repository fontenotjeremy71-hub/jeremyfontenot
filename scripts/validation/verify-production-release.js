...
async function request(url, options = {}, timeout = 60_000) {
  ...
  try {
    return await fetch(url, {
      ...
      headers: {
        "user-agent": "portfolio-production-release-verifier",
...
async function text(url, expectedStatus = 200) {
  ...
  const response = await request(`${url}${separator}release=${targetSha.slice(0, 12)}`);
...
async function verifyHttp() {
  ...
  for (const route of forbiddenRoutes) {
    const response = await request(`${baseUrl}${route}?release=${targetSha.slice(0, 12)}`);
  ...
  try {
    for (const viewport of viewports) {
      ...
      for (const route of ["/systems-skills/evidence-map.html", "/evidence/claim-map.html"]) {
        const response = await page.goto(`${baseUrl}${route}?release=${targetSha.slice(0, 12)}`, {
